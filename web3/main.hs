...
mkYesod "Pagina" [parseRoutes| 
  ...
  /livro/delete/#LivroId DeleteR DELETE 
...
formLivro = renderDivs $ Livro <$>
             ...
             areq (jqueryDayField def { jdsChangeYear = True 
                 , jdsYearRange = "1980:2017"}) "Data" Nothing <*>
...
getHomeR = defaultLayout $ do 
            ...
            ^{header}
            <main>
                <img .imagem2 src=@{StaticR bemvindo_png}>
                ...
                defaultLayout $ do
                    ...
                    <main>
                        ...
                        $forall Entity lid lent <- livro
                            <p> 
                                #{livroNome lent}
                                <a type="button" .book-del data-url=@{ DeleteR lid }>X
                    ^{footer} 
                    |]
                    toWidget [julius|
                        [].slice.call(document.querySelectorAll('.book-del')).map(function(a){ 
                               a.onclick = function(){
                                    ...
                                    httpRequest.open('DELETE', this.dataset.url, true);
                                    httpRequest.send();
